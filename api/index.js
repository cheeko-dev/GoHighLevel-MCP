...
{name:"create_custom_field",description:"Create a custom field. For SINGLE_OPTIONS or MULTIPLE_OPTIONS pass options as array of strings e.g. ['Option1','Option2']",inputSchema:{type:"object",properties:{name:{type:"string"},fieldKey:{type:"string"},dataType:{type:"string",description:"TEXT, LARGE_TEXT, NUMERICAL, PHONE, MONETORY, CHECKBOX, SINGLE_OPTIONS, MULTIPLE_OPTIONS, FLOAT, TIME, DATE"},options:{type:"array",items:{type:"string"},description:"Array of option strings for SINGLE_OPTIONS or MULTIPLE_OPTIONS fields"}},required:["name","fieldKey","dataType"]}},
...
case"get_custom_fields":{const r=await ghlRequest('GET',`/locations/${GHL_LOCATION_ID}/customFields`,'2021-07-28');return JSON.stringify(r.data,null,2);}
case"create_custom_field":{
const body={name:args.name,fieldKey:args.fieldKey,dataType:args.dataType,locationId:GHL_LOCATION_ID,model:'contact'};
if(args.options&&Array.isArray(args.options)){body.options=args.options.map((o,i)=>({key:`${args.fieldKey}_${i}`,label:typeof o==='string'?o:o.label,value:typeof o==='string'?o:o.value||o.label}));}
const r=await ghlRequest('POST',`/locations/${GHL_LOCATION_ID}/customFields`,body);
...
if(req.url==='/'||req.url==='/health'){res.status(200).json({status:'healthy',server:'ghl-mcp-server',version:'4.0.0',protocol:MCP_PROTOCOL_VERSION,timestamp:new Date().toISOString(),toolCount:TOOLS.length,tools:TOOLS.map(t=>t.name),endpoint:'/sse',locationId:GHL_LOCATION_ID,hasApiKey:!!GHL_API_KEY});return;}
